...
//  А вот УКАЗАТЕЛЬ НА НЕГО [node_t*], хранящий адрес на него, может быть           |
...
int main(int argc, const char * argv[]) {
    ...
    node_t *pBegin = new node_t;            // выделяем память для структуры типа node_t
                                            // и кладем ее адрес в указатель pBegin
    node_t *p = pBegin;                     // приваиваем указателю p адрес первой
                                            // структуры, на которую указывает pBbegin
    ...
    for (int i = 0; i < count; ++i) {
        ...
        p->next = new node_t;       // выделяем память под следующую структуру
                                    // кладем в указатель next - он же ПОЛЕ структуры -
                                    // - адрес следующей, вновь созданной структуры
        p = p->next;                // кладем в указатель p адрес только что
                                    // сознанной структуры
    }
...
// высвобождаем выделенную память
    ...
    p = pBegin;
    // проходим по структурам, начиная со второй - присваиваем указателю p
    // адрес, который хранится в первой структуре в ПОЛЕ next и так по цепочке
    // пока указатель p не станет указывать на НУЛЕВОЙ УКАЗАТЕЛЬ nullptr
    while (p != nullptr) {
        p = p->next;
        delete p;
    }
