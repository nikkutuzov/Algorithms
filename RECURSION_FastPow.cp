...
//  -> return fastPow(2, 17 - 1) * 2 -> идет проверка(16 % 2 == 0) ->               |
//  -> return fastPow(2^2, 16 / 2) -> идет проверка(8 % 2 == 0) ->                  |
//  -> return fastPow(4^2, 8 / 2) -> идет проверка(4 % 2 == 0) ->                   |
//  -> return fastPow(16^2, 4 / 2) -> идет проверка(2 % 2 == 0) ->                  |
//  -> return fastPow(256^2, 2 / 2) -> идет проверка(1 % 2 != 0) ->                 |
//  -> return fastPow(65536, 1 - 1) -> идет проверка(0 == 0) ->                     |
//  -> return 1 в fastPow(256^2, 2 / 2), так как fastPow(256^2, 2 / 2) была         |
//  вызвана из условия:                                                             |
//  } else if (power % 2 != 0){     // Если НЕчетное                                |
//          return FastPOW(inputNumber, power - 1) * inputNumber;                   |
//  то есть:                                                                        |
...
//          return FastPOW(256^2, 1 - 1) * 256^2;                                   |
//            ||          ||                 ||                                     |
//            \/          \/                 \/                                     |
//  то есть [return      (1  умножаем  на  65536)] в функцию fastPow(16^2, 4 / 2),  |
//  там, следуя из условия power(то есть 4/2=2) остаток от деления 2 на 2 равен нулю|
//  значит просто return FastPOW(inputNumber * inputNumber, power / 2);             |
//  то есть предыдущее значение 65536 возвращается в функцию fastPow(4^2, 8 / 2),   |
//  а там проверялось условие power(то есть 8/2=4) остаток от деления 4 на 2        |
//  равен нулю, значит просто return FastPOW(inputNumber * inputNumber, power / 2); |
//  то есть предыдущее значение 65536 возвращается в функцию fastPow(2^2, 16 / 2),  |
//  а там проверялось условие power(то есть 16/2=8) остаток от деления 8 на 2       |
//  равен нулю, значит просто return FastPOW(inputNumber * inputNumber, power / 2); |
//  то есть предыдущее значение 65536 возвращается в функцию fastPow(2, 17-1),      |
//  а там проверялось условие power(то есть 17 - 1) остаток от деления 16 на 2 равен|
//  0, то есть предыдущее значение 65536 возвращается в функцию fastPow(2, 17),     |
//  а там проверялось условие power(то есть 17) остаток от деления 17 на 2          |
//  НЕ РАВЕН НУЛЮ, значит вызов функции был из:                                     |
//  } else if (power % 2 != 0){     // Если НЕчетное                                |
//          return FastPOW(inputNumber, power - 1) * inputNumber;                   |
//  то есть:                                                                        |
//                                                                                  |
//  } else if ((17 / 2) % 2 != 0){     // то есть 17 % 2 не равно нулю!             |
//          return FastPOW(2, 17 - 1) * 2;                                          |
//            ||      ||               ||                                           |
//            \/      \/               \/                                           |
//  то есть [return  (65536 умножаем на 2)] в функцию main()                        |
//  65536 * 2 = 131072, проверяем на калькуляторе и получаем 2^17 = 131072.         |
//__________________________________________________________________________________|
