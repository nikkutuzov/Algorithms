...
//  чтобы расставить два предмета в двух позициях   1   2 - либо 1  2 либо 2  1     |
//                                                               ^  ^      ^  ^     |
//                                                              [1][2]    [1][2]    |
//                                              2! = 2, 3! 3 предмета, 3 позиции:   |
...
//  В() и...                    |   call                                            |
//  ..она начинает выполняться->В()-------* <- В() начинае выполняться              |
//                                        |                                         |
//                                        | функция С() выполняется и...            |
//                                        |   call                                  |
...
//                                 return | будет В()  и возвращает результат в В() |
//                              x---------x <- B() завершает выполниение            |
//                       return | будет А()   и возвращает результат в A()          |
...
// функция В() засыпает|_________________________________________.будет функцию B() |
...
//       ||     | автоматически падает адрес возврата в то место |    ||            |
//       ||     | откуда был вызов - нужно знать куда вернуться  |    ||            |
// функция А() засыпает|_________________________________________|.будет функцию A()|
